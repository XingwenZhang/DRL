...
class DQNAgent:
    ...
    def __init__(self, env):
        ...
        self._cur_observation = None
        if config.prioritized_experience_replay:
            self._replay_memory = DQNUtil.PrioritizedExperienceReplayMemory()
        else:
        ...
        self._tf_pn_target = None
        self._tf_tn_target = None
        self._tf_pn_importance_weight = None
        self._tf_pn_delta = None
        self._tf_summary_pn_loss = None
        self._tf_summary_averaged_pn_Q = None
        self._tf_summary_episode_reward = None
        self._tf_episode_reward = None
        self._tf_episode_steps = None
        self._tf_summary_episode_steps = None
        self._saver = None
    ...
    def _init_dqn(self, dueling_dqn):
        ...
        pn_nodes, tn_nodes, cloning_ops, train_summary_ops, evaluation = THORNet.build_network('thor_net', self._env.get_num_actions(), dueling_dqn=dueling_dqn)
        self._tf_pn_state, self._tf_pn_target, self._tf_pn_Q, self._tf_pn_loss, self._tf_pn_actions, self._tf_pn_Q_target, self._tf_pn_train, self._tf_pn_importance_weight, self._tf_pn_delta = pn_nodes
        self._tf_tn_state, self._tf_tn_target, self._tf_tn_Q = tn_nodes
        self._tf_clone_ops = cloning_ops
        self._tf_summary_pn_loss, self._tf_summary_averaged_pn_Q = train_summary_ops
        self._tf_episode_reward, self._tf_summary_episode_reward, self._tf_episode_steps, self._tf_summary_episode_steps = evaluation
        self._saver = tf.train.Saver()

    def _evaluate_q(self, state, target):
        assert(self._tf_sess is not None)
        assert(self._tf_pn_Q is not None)
        assert(target is not None)
        tiled_target = np.tile(target[:,np.newaxis,:], (1,4,1))
        Q = self._tf_sess.run(self._tf_pn_Q, feed_dict={self._tf_pn_state: state, 
                                                        self._tf_pn_target: tiled_target})
        return Q
    ...
    def _perform_action(self, action):
        if config.prioritized_experience_replay:
            pre_state = self._histroy_frames.copy_content()
            self._cur_observation, reward, done, _ = self._env.step(action)
            self._histroy_frames.record(self._cur_observation)
            cur_state = self._histroy_frames.copy_content()
            self._replay_memory.add(pre_state, self._cur_target, action, cur_state, reward, done)
        else:
            self._cur_observation, reward, done, _ = self._env.step(action)
            self._histroy_frames.record(self._cur_observation)
    ...
    def _request_new_episode(self, random_start=False):
        ...
        self._cur_target = self._env.get_target_feat()
    ...
    def decompose_experiences(experiences):
        ...
        targets = np.array([experience[1] for experience in experiences])
        targets = np.tile(targets[:,np.newaxis,:], (1,4,1))
        actions = np.array([experience[2] for experience in experiences])
        states_new = np.array([experience[3] for experience in experiences])
        rewards = np.array([experience[4] for experience in experiences])
        dones = np.array([experience[5] for experience in experiences])
        return states, targets, actions, states_new, rewards, dones
